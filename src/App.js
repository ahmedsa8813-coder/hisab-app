...
export default function App() {
  ...
  const deleteProject = async (id) => {
    if (window.confirm("تريد تحذف هذا المشروع؟")) {
      await deleteDoc(doc(db, "projects", id));
    }
  };

  const deleteTransaction = async (id) => {
    if (window.confirm("تريد تحذف هذه المعاملة؟")) {
  ...
  // Statement for selected user
  const statementTx = selectedUserStatement
    ...
    <div style={S.page}>
      <div style={S.appWrap}>
        <div style={S.header}>
          ...
          <button style={S.logoutBtn} onClick={() => { setUser(null); setScreen("login"); setLoginId(null); setPin(""); setView("home"); }}>خروج</button>
        </div>
        ...
        <div style={S.content}>
          ...
          {!loading && user.role === "manager" && view === "home" && (
            <div>
              ...
              {employeeBalances.map(e => (
                <button key={e.id} style={S.empCard} onClick={() => { setSelectedUserStatement(e.id); setFilterFrom(""); setFilterTo(""); setView("statement"); }}>
                  <div style={S.empTop}>
                    <div style={{ ...S.avatar, width:40, height:40, fontSize:17 }}>{e.name[0]}</div>
                    <div style={{ flex: 1 }}>
          ...
          {!loading && user.role === "manager" && view === "statement" && (
            <div>
              <div style={S.secTitle}>كشف حساب - {USERS.find(u=>u.id===selectedUserStatement)?.name}</div>
