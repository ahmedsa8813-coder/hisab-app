...
export default function App() {
  ...
  const addProject = async () => {
    ...
    await addDoc(collection(db,"projects"),{
      name:newProject.name.trim(), specialization:newProject.specialization,
      province:newProject.province, value:Number(newProject.value||0), currency:newProject.currency,
    });
  ...
  const getProjectReport = (proj, from, to) => {
    if(!proj) return null;
    const projTx = transactions.filter(t => {
      if(t.projectId !== proj.id) return false;
      if(t.type !== "صرف") return false;
    ...
    const byEmployee = USERS.filter(u=>u.role==="employee").map(u=>{
      const uTx = projTx.filter(t=>t.userId===u.id);
      const spent = uTx.reduce((s,t)=>s+t.amount,0);
      return{...u,spent,txCount:uTx.length,transactions:uTx};
    }).filter(u=>u.spent>0);
  ...
  const exportPersonPDF = () => {
    ...
    const rows=statementTxRaw.map(t=>`<tr><td>${t.date}</td><td>${t.projectName}</td><td style="color:${t.type==='استلام'?'green':'red'}">${t.type}</td><td>${t.type==='استلام'?toAr(Number(t.amount).toLocaleString("ar-IQ")):'-'}</td><td>${t.type==='صرف'?toAr(Number(t.amount).toLocaleString("ar-IQ")):'-'}</td><td>${t.note||"-"}</td></tr>`).join("");
    const html=`<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"/><title>كشف حساب - ${statementUser?.name}</title>
    <style>body{font-family:Tahoma,sans-serif;padding:30px;color:#111;direction:rtl}h1{color:#1d4ed8;font-size:20px}.info{font-size:13px;color:#555;margin:8px 0 20px}.summary{display:flex;gap:12px;margin-bottom:24px}.box{border:1px solid #ddd;border-radius:8px;padding:12px;flex:1;text-align:center}.label{font-size:11px;color:#888}.val{font-size:16px;font-weight:bold;margin-top:4px}.green{color:#047857}.red{color:#991b1b}table{width:100%;border-collapse:collapse;font-size:12px}th{background:#1d4ed8;color:white;padding:8px}td{padding:7px 8px;border-bottom:1px solid #eee;text-align:center}tr:nth-child(even){background:#f9f9f9}.footer{margin-top:24px;font-size:11px;color:#aaa;text-align:center}</style></head>
    <body><h1>كشف حساب - ${statementUser?.name}</h1>
    ...
    <div class="footer">نظام حساب - ${new Date().toLocaleDateString("ar-IQ")}</div></body></html>`;
  ...
  const exportProjectPDF = () => {
    ...
    const html=`<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"/><title>الكشف المالي - ${proj.name}</title>
    <style>body{font-family:Tahoma,sans-serif;padding:30px;color:#111;direction:rtl}h1{color:#1d4ed8;font-size:22px}h2{color:#374151;font-size:16px;margin-top:24px}.info{font-size:13px;color:#555;margin:8px 0 20px}.summary{display:flex;gap:12px;margin-bottom:20px}.box{border:1px solid #ddd;border-radius:8px;padding:12px;flex:1;text-align:center}.label{font-size:11px;color:#888}.val{font-size:15px;font-weight:bold;margin-top:4px}.green{color:#047857}.red{color:#991b1b}.orange{color:#d97706}.progress{background:#e5e7eb;border-radius:999px;height:12px;margin:8px 0}.progress-bar{background:#1d4ed8;border-radius:999px;height:12px}table{width:100%;border-collapse:collapse;font-size:12px}th{background:#1d4ed8;color:white;padding:8px}td{padding:7px 8px;border-bottom:1px solid #eee;text-align:center}tr:nth-child(even){background:#f9f9f9}.footer{margin-top:24px;font-size:11px;color:#aaa;text-align:center}</style></head>
    <body>
    <h1>الكشف المالي للمشروع</h1>
    <div class="info">${proj.name} - ${proj.specialization} - ${proj.province}${projFilterFrom?` | من: ${projFilterFrom}`:""}${projFilterTo?` | إلى: ${projFilterTo}`:""}</div>
    <div class="summary">
      <div class="box"><div class="label">قيمة المشروع</div><div class="val blue">${fmtD(proj.value)}</div></div>
      <div class="box"><div class="label">إجمالي المصروف</div><div class="val red">${fmtD(totalSpent)}</div></div>
      <div class="box"><div class="label">المتبقي</div><div class="val ${remaining>=0?'green':'red'}">${fmtD(Math.abs(remaining))} ${remaining>=0?'':'تجاوز'}</div></div>
      <div class="box"><div class="label">نسبة الصرف</div><div class="val orange">${toAr(pct)}%</div></div>
    </div>
    <div class="progress"><div class="progress-bar" style="width:${pct}%"></div></div>
    <h2>تفصيل المصروفات بالموظفين</h2>
    <table><thead><tr><th>الموظف</th><th>إجمالي الصرف</th><th>عدد المعاملات</th><th>النسبة من المشروع</th></tr></thead>
    <tbody>${empRows}</tbody></table>
    <h2>تفاصيل المعاملات</h2>
    <table><thead><tr><th>التاريخ</th><th>الموظف</th><th>المبلغ</th><th>ملاحظات</th></tr></thead>
        ...
        {!loginId?(
          <>
            ...
            <div style={D?S.userGridD:S.userGrid}>
              {USERS.map(u=>(
                <button key={u.id} style={{...S.userBtn,...(u.role==="manager"?S.managerBtn:{})}} onClick={()=>{setLoginId(u.id);setPin("");setPinError(false);}}>
          ...
          <>
            <div style={S.selectedUser}>
              <div style={{...S.avatar,background:USERS.find(u=>u.id===loginId)?.role==="manager"?"linear-gradient(135deg,#1d4ed8,#2563eb)":"linear-gradient(135deg,#f59e0b,#d97706)"}}>{USERS.find(u=>u.id===loginId)?.name[0]}</div>
              <div><div style={{fontSize:16,fontWeight:700}}>{USERS.find(u=>u.id===loginId)?.name}</div><div style={{fontSize:12,color:"#9ca3af"}}>{USERS.find(u=>u.id===loginId)?.role==="manager"?"مدير مالي":"حسابات"}</div></div>
            </div>
  ...
  const navItems = user?.role==="employee"
    ...
    : [{icon:"📊",label:"الملخص",v:"home"},{icon:"📋",label:"المعاملات",v:"allTx"},{icon:"🏗️",label:"المشاريع",v:"projects"},{icon:"💰",label:"الكشف المالي",v:"projReport"},{icon:"⚖️",label:"افتتاحي",v:"opening"}];
      ...
      <div style={D?S.appWrapD:S.appWrap}>
        <div style={D?S.headerD:S.header}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div><div style={S.headerName}>{user.name}</div><div style={S.headerRole}>{user.role==="manager"?"مدير مالي":"حسابات - "+user.name}</div></div>
          </div>
        ...
        <div style={D?S.appBodyD:{}}>
          {D&&(
            <div style={S.sidebar}>
              {navItems.map(n=><button key={n.v} style={{...S.sideBtn,...(view===n.v?S.sideBtnActive:{})}} onClick={()=>setView(n.v)}><span>{n.icon}</span><span>{n.label}</span></button>)}
            </div>
          )}
          ...
          <div style={D?S.mainContent:S.content}>
              ...
              <div style={D?{maxWidth:600}:{}}>
                ...
                {formSuccess?<div style={S.successBox}><div style={S.successIcon}>✓</div><div>تم التسجيل!</div></div>:(
                  <div style={S.formCard}>
                    ...
                    <div style={S.typeRow}>{["استلام","صرف"].map(t=><button key={t} style={{...S.typeBtn,...(form.type===t?(t==="استلام"?S.typeBtnGreen:S.typeBtnRed):{})}} onClick={()=>setForm(f=>({...f,type:t}))}>{t==="استلام"?"↓ استلام":"↑ صرف"}</button>)}</div>
            ...
            {!loading&&user.role==="manager"&&view==="home"&&(
              <div>
                ...
                <div style={D?S.empGridD:{}}>{employeeBalances.map(e=>(
                  <button key={e.id} style={S.empCard} onClick={()=>{setSelectedUserStatement(e.id);setFilterFrom("");setFilterTo("");setFilterProject("all");setFilterCurrency("دينار");setView("statement");}}>
                    <div style={S.empTop}>
                      ...
                      <div style={{flex:1}}><div style={S.empName}>{e.name}</div><div style={S.empSub}>{toAr(e.txCount)} معاملة</div></div>
                ...
                {!D&&<>
                  ...
                  <button style={{...S.addBtn,background:"linear-gradient(135deg,#b45309,#92400e)",marginTop:10}} onClick={()=>setView("projReport")}>💰 الكشف المالي للمشاريع</button>
              ...
              <div>
                ...
                <div style={D?S.statementLayoutD:{}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    {(obRec||obSp)>0&&<div style={S.obBadge}>⚖️ رصيد افتتاحي: استلم {fmt(obRec,filterCurrency)} · صرف {fmt(obSp,filterCurrency)}</div>}
            ...
            {!loading&&user.role==="manager"&&view==="projects"&&(
              <div style={D?{maxWidth:700}:{}}>
                ...
                <div style={S.formCard}>
                  ...
                  <div style={D?S.rowD:{}}>
                    <div style={D?{flex:2}:{}}>
                      <div style={S.fieldLabel}>قيمة المشروع</div>
                      <input style={S.input} type="number" placeholder="٠" value={newProject.value} onChange={e=>setNewProject(p=>({...p,value:e.target.value}))}/>
                    </div>
                    <div style={D?{flex:1}:{}}>
                      <div style={S.fieldLabel}>عملة المشروع</div>
                      <div style={S.typeRow}>{["دينار","دولار"].map(c=><button key={c} style={{...S.typeBtn,...(newProject.currency===c?S.typeBtnBlue:{})}} onClick={()=>setNewProject(p=>({...p,currency:c}))}>{c==="دينار"?"🇮🇶":"🇺🇸"} {c}</button>)}</div>
                    </div>
                  </div>
                ...
                <div style={D?S.txGridD:{}}>{projects.map(p=>(
                  <div key={p.id} style={S.projCard}>
                    <div>
                      ...
                      {p.value>0&&<div style={{fontSize:12,color:"#f59e0b",marginTop:3}}>قيمة المشروع: {fmtD(p.value)}</div>}
                    </div>
            ...
            {!loading&&user.role==="manager"&&view==="projReport"&&(
              <div>
                ...
                {!selectedProject?(
                  <div>
                    <div style={{fontSize:13,color:"#9ca3af",marginBottom:16}}>اختر مشروع لعرض كشفه المالي</div>
                      ...
                      return(
                        <button key={p.id} style={S.projReportCard} onClick={()=>{setSelectedProject(p);setProjFilterFrom("");setProjFilterTo("");}}>
                          ...
                          {p.value>0&&<>
                            ...
                            <div style={{fontSize:11,color:"#9ca3af",marginTop:4}}>قيمة المشروع: {fmtD(p.value)} · {toAr(pct)}% مصروف</div>
                          </>}
                        </button>
                      );
                    })}</div>
                  ...
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
                      ...
                      <div style={{fontWeight:700,fontSize:16}}>{selectedProject.name} - {selectedProject.specialization} - {selectedProject.province}</div>
                    </div>
                    <div style={D?S.statementLayoutD:{}}>
                      ...
                      <div style={{flex:1}}>
                        {projReport&&<>
                          ...
                          <div style={D?S.cardsRowD:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
                            <div style={{...S.miniCard,background:"linear-gradient(135deg,#1d4ed8,#2563eb)"}}>
                              <div style={S.miniLabel}>قيمة المشروع</div>
                              <div style={S.miniVal}>{fmtD(selectedProject.value)}</div>
                            </div>
                            <div style={{...S.miniCard,background:"linear-gradient(135deg,#7f1d1d,#991b1b)"}}>
                              <div style={S.miniLabel}>إجمالي المصروف</div>
                              <div style={S.miniVal}>{fmtD(projReport.totalSpent)}</div>
                            </div>
                            <div style={{...S.miniCard,background:projReport.remaining>=0?"linear-gradient(135deg,#065f46,#047857)":"linear-gradient(135deg,#7f1d1d,#991b1b)"}}>
                              <div style={S.miniLabel}>{projReport.remaining>=0?"المتبقي":"تجاوز الميزانية"}</div>
                              <div style={S.miniVal}>{fmtD(Math.abs(projReport.remaining))}</div>
                            </div>
                            <div style={{...S.miniCard,background:"linear-gradient(135deg,#b45309,#92400e)"}}>
                              <div style={S.miniLabel}>نسبة الصرف</div>
                              <div style={S.miniVal}>{selectedProject.value?toAr(Math.min(100,Math.round(projReport.totalSpent/selectedProject.value*100)))+"%":"—"}</div>
                            </div>
                          </div>
                          {/* Progress */}
                          {selectedProject.value>0&&<div style={{marginBottom:20}}>
                            <div style={S.progressBar}>
                              <div style={{...S.progressFill,width:`${Math.min(100,Math.round(projReport.totalSpent/selectedProject.value*100))}%`}}/>
                            </div>
                          </div>}
                          {/* By employee */}
                          <div style={S.secTitle}>تفصيل مصروفات الموظفين</div>
                          {projReport.byEmployee.length===0?<div style={S.empty}>ما في مصروفات</div>:projReport.byEmployee.map(e=>(
                            <div key={e.id} style={S.empSpendCard}>
                              <div style={{display:"flex",alignItems:"center",gap:10}}>
                                <div style={{...S.avatar,width:36,height:36,fontSize:15,margin:0}}>{e.name[0]}</div>
                                <div style={{flex:1}}>
                                  <div style={{fontWeight:600,fontSize:14}}>{e.name}</div>
                                  <div style={{fontSize:11,color:"#6b7280"}}>{toAr(e.txCount)} معاملة</div>
                                </div>
                                <div style={{textAlign:"left"}}>
                                  <div style={{fontSize:15,fontWeight:700,color:"#f87171"}}>{fmtD(e.spent)}</div>
                                  {selectedProject.value>0&&<div style={{fontSize:11,color:"#9ca3af"}}>{toAr(Math.round(e.spent/selectedProject.value*100))}% من المشروع</div>}
                                </div>
                              </div>
                              <div style={{marginTop:8}}>
                                <div style={{...S.progressBar,height:6}}>
                                  <div style={{...S.progressFill,width:`${selectedProject.value?Math.min(100,Math.round(e.spent/selectedProject.value*100)):0}%`,background:"#f87171"}}/>
                                </div>
                              </div>
                            </div>
                          ))}
            ...
            {!loading&&user.role==="manager"&&view==="opening"&&(
              <div style={D?{maxWidth:900}:{}}>
                ...
                <div style={D?S.empGridD:{}}>{USERS.filter(u=>u.role==="employee").map(u=>{
                  const ob=openingBalances[u.id]||{};
                  const of=openingForm[u.id]||{};
                  return(
                    <div key={u.id} style={S.obCard}>
                      <div style={S.obHeader}><div style={{...S.avatar,width:36,height:36,fontSize:16,margin:0}}>{u.name[0]}</div><div style={{fontWeight:700,fontSize:15}}>{u.name}</div></div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:12}}>
                        {[["dinarReceived","🇮🇶 دينار استلم"],["dinarSpent","🇮🇶 دينار صرف"],["dollarReceived","🇺🇸 دولار استلم"],["dollarSpent","🇺🇸 دولار صرف"]].map(([k,l])=>(
                          <div key={k}><div style={{fontSize:11,color:"#9ca3af",marginBottom:4}}>{l}</div>
                          <input style={{...S.input,fontSize:13,padding:"8px 10px"}} type="number" placeholder={toAr(ob[k]||0)} value={of[k]??""} onChange={e=>setOpeningForm(f=>({...f,[u.id]:{...(f[u.id]||{}),[k]:e.target.value}}))}/>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        {!D&&(
          <div style={S.bottomNav}>
            {navItems.map(n=><button key={n.v} style={{...S.navBtn,...(view===n.v?S.navActive:{})}} onClick={()=>setView(n.v)}><span>{n.icon}</span><span style={{fontSize:10}}>{n.label}</span></button>)}
          </div>
        )}
      </div>
    </div>
...
const S={
  ...
  miniCard:{borderRadius:16,padding:"16px",flex:1},
  ...
  bottomNav:{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:420,background:"rgba(10,10,15,0.97)",borderTop:"1px solid rgba(255,255,255,0.08)",display:"flex",padding:"8px 8px 18px"},
