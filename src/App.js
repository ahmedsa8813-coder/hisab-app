...
export default function App() {
  ...
  const exportPDF = () => {
    ...
    const fromDate = filterFrom || "البداية";
    const toDate = filterTo || "الآن";
    ...
    const rows = statementTx.map(t => `
      <tr>
        ...
        <td>${Number(t.amount).toLocaleString("ar-IQ")}</td>
        <td>${t.note || "-"}</td>
      </tr>
    `).join("");
    ...
    const html = `
      ...
      <body>
        ...
        <div class="info">
          المشروع: ${projName} &nbsp;|&nbsp; من: ${fromDate} &nbsp;|&nbsp; إلى: ${toDate}
        </div>
        <div class="summary">
          <div class="box"><div class="label">إجمالي الاستلام</div><div class="val green">${fmt(statementReceived)}</div></div>
          <div class="box"><div class="label">إجمالي الصرف</div><div class="val red">${fmt(statementSpent)}</div></div>
          <div class="box"><div class="label">الرصيد</div><div class="val ${statementBalance>=0?'green':'red'}">${fmt(Math.abs(statementBalance))} ${statementBalance>=0?'متبقي':'عليه'}</div></div>
        </div>
        <table>
          <thead><tr><th>التاريخ</th><th>المشروع</th><th>النوع</th><th>المبلغ (د.ع)</th><th>ملاحظات</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
        <div class="footer">نظام حساب - تم الإنشاء بتاريخ ${new Date().toLocaleDateString("ar-IQ")}</div>
      </body>
      </html>
    `;
        ...
        {!loginId ? (
          ...
          <>
            <div style={S.selectedUser}>
              ...
              <div style={{ fontSize:18, fontWeight:700 }}>{USERS.find(u=>u.id===loginId)?.name}</div>
            </div>
      ...
      <div style={S.appWrap}>
        <div style={S.header}>
          <div>
            ...
            <div style={S.headerRole}>{user.role==="manager"?"مدير مالي":"موظف"}</div>
          </div>
        ...
        <div style={S.content}>
          ...
          {!loading && user.role==="employee" && view==="home" && (
            <div>
              ...
              <button style={S.addBtn} onClick={() => { setView("add"); setForm({ type:"استلام", projectId:"", amount:"", note:"", date:new Date().toISOString().split("T")[0] }); }}>+ تسجيل معاملة جديدة</button>
          ...
          {!loading && user.role==="manager" && view==="home" && (
            <div>
              ...
              {employeeBalances.map(e => (
                <button key={e.id} style={S.empCard} onClick={() => { setSelectedUserStatement(e.id); setFilterFrom(""); setFilterTo(""); setFilterProject("all"); setView("statement"); }}>
                  <div style={S.empTop}>
                    ...
                    <div style={{ textAlign:"center" }}>
                      <div style={{ fontSize:11, color:"#9ca3af" }}>{e.balance>=0?"متبقي":"عليه"}</div>
                      <div style={{ fontSize:14, fontWeight:700, color:e.balance>=0?"#34d399":"#f87171" }}>{fmt(Math.abs(e.balance))}</div>
                    </div>
                    <div style={{ color:"#6b7280", marginRight:8 }}>←</div>
                  </div>
                </button>
              ))}
              <div style={{ height:12 }} />
          ...
          {!loading && user.role==="manager" && view==="statement" && (
            <div>
              ...
              <div style={{ ...S.balCard, background:statementBalance>=0?"linear-gradient(135deg,#065f46,#047857)":"linear-gradient(135deg,#7f1d1d,#991b1b)", marginBottom:16 }}>
                <div style={S.balLabel}>الرصيد</div>
                <div style={S.balAmount}>{fmt(Math.abs(statementBalance))}</div>
                <div style={S.balSub}>{statementBalance>=0?"متبقي معه":"عليه"}</div>
                <div style={S.balRow}>
                  <span style={S.balStat}>↓ استلم {fmt(statementReceived)}</span>
                  <span style={S.balStat}>↑ صرف {fmt(statementSpent)}</span>
                </div>
              </div>
          ...
          {!loading && user.role==="manager" && view==="allTx" && (
            <div>
              ...
              <div style={{ fontSize:13, color:"#9ca3af", marginBottom:12 }}>{filteredTx.length} معاملة</div>
...
const S = {
  ...
  submitBtn:{ width:"100%", background:"linear-gradient(135deg,#f59e0b,#d97706)", border:"none", borderRadius:14, padding:"16px", color:"#000", fontSize:16, fontWeight:700, cursor:"pointer" },
